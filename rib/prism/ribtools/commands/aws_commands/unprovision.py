...
class Unprovision(RIBCommand):
    """Unprovision the active environment.."""
    ...
    def run(self):
        e = self.ensure_aws()
        cmd = self.environment_boilerplate(e, "unprovision")
        ...
        if result.returncode:
            raise PRTError(f"Something went wrong unprovisioning environment {e.name}")
