    ...
    while True:
        ...
        for otherRow in range(0, numRows):
            ...
            if matrix[otherRow][j] != 0:
                matrix[otherRow] = [y - matrix[otherRow][j] * x for (x, y) in zip(matrix[i], matrix[otherRow])]
...
def someSolution(system, freeVariableValue=1):
    ...
    for j in pivotVars:
        ...
        variableValues[j] = (system[theRow][-1] - sum(system[theRow][i] * variableValues[i] for i in freeVars))
